...
#include <vector>
#include <cassert>
#include "KKousour/TopAnalysis/plugins/JMEFlatTreeProducer.h"//need to edit
...
JMEFlatTreeProducer::JMEFlatTreeProducer(edm::ParameterSet const& cfg): 
  ...
  metFilterResultsToken(consumes<edm::TriggerResults>(cfg.getParameter<edm::InputTag>("metFilterResults"))),
  ...
  metFilterNames_(cfg.getParameter<std::vector<std::string> >("metFilterNames")),
...
void JMEFlatTreeProducer::beginJob() 
{
  ...
  //--- book the met filter histograms ---------
  metFilterNamesHisto_ = fs_->make<TH1F>("metFilterNames","metFilterNames",1,0,1);
  metFilterNamesHisto_->SetCanExtend(TH1::kAllAxes);
  for(unsigned i=0;i<metFilterNames_.size();i++) {
    metFilterNamesHisto_->Fill(metFilterNames_[i].c_str(),1);
  }
  metFilterPassHisto_ = fs_->make<TH1F>("metFilterPass","metFilterPass",1,0,1);
  metFilterPassHisto_->SetCanExtend(TH1::kAllAxes);
  //--- book the tree -----------------------
  ...
  outTree_->Branch("llMass"               ,&llMass_            ,"nLeptons_/F");
  //------------------------------------------------------------------
...
//////////////////////////////////////////////////////////////////////////////////////////
bool JMEFlatTreeProducer::isGoodMuon(const pat::Muon &mu,const reco::Vertex &vtx,float rho)
{
...
//////////////////////////////////////////////////////////////////////////////////////////
float JMEFlatTreeProducer::MuonRelIso(const reco::Candidate *cand,float rho)
{
...
void JMEFlatTreeProducer::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) 
{
  ...
  //-------------- MET filter Info -----------------------------------
  metFilterPassHisto_->Fill("totalEvents",1);
  const edm::TriggerNames &namesFilter = iEvent.triggerNames(*metFilterResults);  
  for(unsigned int k=0;k<metFilterNames_.size();k++) {
    bool bit(false);
    for(unsigned int ifilt=0;ifilt<metFilterResults->size();ifilt++) {
      string metFilter_name = string(namesFilter.triggerName(ifilt));
      if (metFilter_name == metFilterNames_[k]) {
        bit = metFilterResults->accept(ifilt); 
        if (bit) {
          metFilterPassHisto_->Fill(metFilterNames_[k].c_str(),1);
        } 
      }
    }
    metFilterBit_->push_back(bit); 
  }
